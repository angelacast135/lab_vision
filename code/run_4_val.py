...
def read_dataset():
    ...
    for i, file in enumerate(glob.glob(os.path.join("../BSR/BSDS500/data/images/val/*.jpg"))):
...
def k_means( im , k , gain_vals ):
    ...
    gain = np.ones([im_vector.shape[0],1])
    gain[0] = gain_vals
    im_vector = np.multiply(im_vector,gain)
...
def segmentByClustering( rgbImage, featureSpace, clusteringMethod, numberOfClusters, gain):
    ...
    if clusteringMethod == 'kmeans':
        ...
        im_segmented = k_means(im_feat,numberOfClusters, gain)
...
if not  os.path.isdir("./val"):
    os.system('mkdir ./val')
    os.system('mkdir ./val/kmeans')
    os.system('mkdir ./val/kmeans/lab')
    os.system('mkdir ./val/kmeans/lab+xy')
    os.system('mkdir ./val/kmeans/lab+xy/1.2')
    os.system('mkdir ./val/kmeans/lab+xy/1.5')
    os.system('mkdir ./val/kmeans/lab+xy/2')


featureSpaces =   ['lab','lab+xy'] #, 'hsv+xy']
...
for i in range(length_fold): #range(4): 
    ...
    for ff , featureSpace in enumerate(featureSpaces):
        for cc, clusteringMethod in enumerate(clusteringMethods):
            ...
            if featureSpace == 'lab+xy':                
                gain_vals=[1.2, 1.5, 2]
                for j, gg in enumerate(gain_vals):
                    for kk, n_cluster in enumerate(numberOfClusters):

                        print("The number of cluster in current image is {}".format(n_cluster))

                        gain = gg
                        im_seg = segmentByClustering( im, featureSpace , clusteringMethod , n_cluster, gain)
                        im_seg_u16 = np.array(im_seg, dtype=np.uint16)
                        obj_arr[kk] = im_seg_u16

                    spio.savemat('./val/'+clusteringMethod+'/'+featureSpace+'/'+str(gg)+'/'+file_name+'.mat', {'segs':obj_arr})
            else:
                for kk, n_cluster in enumerate(numberOfClusters):
                ...
                spio.savemat('./val/'+clusteringMethod+'/'+featureSpace+'/'+file_name+'.mat', {'segs':obj_arr})
